...
理解阶段的职责是**分析用户回复的语义，将其归类为四种标准类型之一**，为 Phase 5（行动）提供决策依据。
...
| **修改请求** | `modification_request` | 用户对计划、优先级、节奏提出调整。语气中性，关注点在行动层面。修改的是**已有**计划，而非创建新计划。 | 「下午的优先级变了，先做 B 项目。」「明天能不能改成 8 点？」「暂停今天的提醒。」「之前说的考公推迟一个月。」 |
| **深度问题** | `deep_question` | 用户提出需要推理、分析、多步拆解的问题。涉及价值选择、复杂决策、长期方向、人际关系等。 | 「我在想是不是该换方向了。」「这个项目和另一个项目冲突了，不知道先做哪个。」 |

### 新增分类的优先规则

4. **意图优先**：如果用户表达了未来要做某事的**具体意图**（有时间、有行动、有计划），且这**不是修改已有计划的参数**（如「明天的节奏改一下」），而是**创造一个新的未来承诺**（如「我打算下个月开始备考」），优先归为 `create_intent`。区分标准：「改已有计划的结构/参数」= `modification_request`；「创造新的未来承诺/项目/目标」= `create_intent`。

...
3. 筛选条件（与 Phase 3 Step 2 完全一致）：
   - 发件人匹配用户邮箱
   - send_time 在 last_sent_timestamp 之后
   - 非系统邮件
   - mailid 不在去重集合中
4. 如果找到 ≥1 封符合条件的未处理用户邮件：
   → 取出第一封有效回复
   → 取消本次 Cron 的主动发送
   → 跳转到 Phase 4（理解）+ Phase 5（行动）处理用户回复
   → 处理完成后进入 Phase 6（闭环），并在下一轮 Cron 时恢复正常的主动触达
5. 如果没有未处理邮件：
   → 继续 Phase 1 Step 5（发送决策）
```

**为什么选「取第一封」而非「全处理」**：
- 与 Phase 3 的设计保持一致：一次会话只处理一封有效回复。
- 如果用户连续发了两封邮件，只处理最新到达的那封（list_mail 按时间倒序返回，第一封 = 最新）。
- 全处理会打破 Cron 的节奏——Agent 可能在 10:30 的 Cron 槽位里陷入处理三封邮件+做计划调整的漫长流程，延误全天调度。

...
- **全量扫描 + 全局去重**：部署环境（agently-cli）的收件箱**长期保留历史邮件**，不会自动归档。轮询不能只按 `begin_time` 窗口或只看「本槽回复」——每个槽位**全量扫描**（limit 50，必要时翻页），与 `memory/daily/*.md` 中**全部** daily 文件已记录的 `message_id` 做**全局去重**（跨天比对，防误日期文件）。所有未处理邮件（含用户主动发送、主题非「回复：」的，如「8.4安排」）一律按意图处理。只比对单日文件会重复处理历史邮件 → 重复建滴答任务、重复发确认。
- **message_id 校验**：列表返回的 ID 偶发与真实 ID 相差一个字符；`msg_` 开头且长度 ≥36 才是完整 ID，短 ID 不能当已处理锚点。处理前用读信接口验证候选 ID，404 且存在近似的已锚定 ID 时按已处理对待，不重复处理。
- **处理锚点**：每封处理完必须把 `message_id` + 处理结果写回当日 daily 文件，作为下一槽位的去重锚点；前台手工补处理时同样要写锚点，否则下个槽会再处理一次（幂等大多能兜住，但会多发确认邮件或重复 complete）。
...
**晚间复盘——休息日版（23:00 周末/节假日）：**

| 变量 | 值 |
|------|---|
| `TIME_LABEL` | 23:00 晚间回顾 |
| `CONTEXT_INTRO` | 不打扰白天，只在睡前留一个轻量回声。 |
| `CORE_QUESTION` | 今天休息得怎么样？ |
| `OPTIONS_LIST` | `<li>休息得不错，[这件事] 让我放松了一下</li><li>还行，明天想试试 [这件小事]</li><li>晚安就好</li>` |

**休息日晨间启动（10:00 休息日默认模式）：**

| 变量 | 值 |
|------|---|
| `TIME_LABEL` | 10:00 休息日晨间 |
| `CONTEXT_INTRO` | 早上好！今天不用赶时间，慢慢来。 |
| `CORE_QUESTION` | 今天有你之前提到的 __（承诺），有想推进的吗？ |
| `OPTIONS_LIST` | `<li>今天想推进 [这件事]</li><li>今天不想做什么，纯放松</li><li>只想做 [这一件小事]</li>` |

**休息日午后跟进（14:30 休息日默认模式）：**

| 变量 | 值 |
|------|---|
| `TIME_LABEL` | 14:30 午后跟进 |
| `CONTEXT_INTRO` | 下午好，来看看今天的情况。 |
| `CORE_QUESTION` | 上午有推进什么吗？还是选择放松？ |
| `OPTIONS_LIST` | `<li>上午做了 [这件事]，下午还想做 [这个]</li><li>今天选择放松，挺好的</li><li>下午想开始做 [这件小事]</li>` |

**纯休息模式问候（10:00 纯休息模式）：**

| 变量 | 值 |
|------|---|
| `TIME_LABEL` | 10:00 早安问候 |
| `CONTEXT_INTRO` | 早安！今天{rest_day 内容，如"去南京"}，玩得开心！ |
| `CORE_QUESTION` | 今天有什么日程需要我提醒的吗？ |
| `OPTIONS_LIST` | `<li>有的，_:__ 的 ___ 别忘了提醒我</li><li>没有，纯放松的一天</li>` |

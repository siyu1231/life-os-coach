...
| E1 | 20:00 | 第一层：① commitments 四类标签 ② TELOS 晚间可执行步骤 ③ 习惯缺口 ④ 邮件提取意图 ⑤ 昨日计划未执行块（轻处置补块）；第二层：四类覆盖缺口兜底 | 晚间三块各 1 件（共 3 件），按类别推荐 | 用户不回按填充默认执行 |
...
1. **昨日未执行块补块**：E1 填充第一层候选增加「昨日计划了但未执行的块」（对比昨日 daily 晚间段「填充方案」与「实际执行」，如昨晚计划行测没做 → 今晚第一块优先排行测）。
2. **每日承诺连续跳过检测**：每日承诺（行测 #6 / 锻炼 #9）连续 2 天未执行 → 计入覆盖缺口（E1 优先推荐），连续跳过原因不追问。

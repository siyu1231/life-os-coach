...
|---|--------|---------|---------|---------|------|
| 1 | **09:30** | Fixed Cron | 仅工作日 | 早安问候 + 今日计划引导 | 发送今日天气、日历摘要、待办概览，询问今日关注点 |
| 2 | **10:30** | Fixed Cron | 仅工作日 | 上午状态检查 | 轻量脉冲：进度如何？精力状态？需要调整优先级吗？ |
| 3 | **14:30** | Fixed Cron | 仅工作日 | 午后启动引导 | 午休后重新聚焦：下午最重要的 1-2 件事 |
| 4 | **15:30** | Fixed Cron | 仅工作日 | 下午中段状态检查 | 轻量脉冲：进度和精力检查，是否需要切换任务类型 |
| 5 | **16:30** | Fixed Cron | 仅工作日 | 收尾预备 | 距离日终 1 小时：今天还有什么必须在结束前推进的？ |
| 6 | **17:30** | Fixed Cron | 仅工作日 | 日终收尾引导 | 日终总结引导：今天完成了什么？偏离在哪里？明天注意什么？ |
| 7 | **22:30** | Fixed Cron | 每日 | 晚间复盘 | 工作日：回顾今日执行与情绪，捕捉觉察；休息日：轻量回顾「今天休息得怎么样？」 |
| 8 | **发送后+5min** | Dynamic Timer | 动态（发送后） | 轻量检查 | 用户未回复时，发送一条温和提醒：「刚才的消息看到了吗？不着急，想到了随时回。」 |
| 9 | **发送后+10min** | Dynamic Timer | 动态（发送后） | 归档等待 | 用户仍未回复时，不再催促，归档本次交互为「等待用户回复」 |
...
所有日间节点（09:30 / 10:30 / 14:30 / 15:30 / 16:30 / 17:30）**静默跳过**，不发送任何消息。

...
7. **邮件正文统一约定：最小执行版本（部署实测 2026-08）**：每封邮件中每出现一个具体任务/块（主线、备选、明日最重要 1 件、晚间填充块、待处置项），各配一行「最小版本：____」——2 分钟内可开始的第一步（如「打开X写第一行」「做3题」「发一条消息」；拿不准用通用版「打开/写下第一步，做5分钟即可停」），并附「做完最小版本就算完成今天这份，回『完成了』或说下实际做了多少即可」（2026-08-19 修订：原「回『启动了』」引导用户实际不会这么回，真实表述是「我完成了最小版本/做了X/只做了X」）。晚间块按类别给最小版本：行测=3 题起步、锻炼=10 分钟拉伸或一组动作、副业=发一条消息或上架一个商品、休闲=随你。目的：状态不好时也能执行最小动作保持状态，邮件从提醒变启动包；用户已回完成情况（含最小版本）的任务不再附（沿用不追问原则）。
   **完成情况报告记账（2026-08-19 定，11 个检查槽部署）**：用户回「我完成了最小版本」「只做了X」「做了3题」= 完成报告（**不是中间状态**）：检查槽 `update_task` 把任务修改为用户实际完成情况（标题追加实绩，如「晚间第一块：行测刷题·已做3题（最小版本）」）→ `complete_task` 勾掉（当天账目清）→ 未完成部分静默后续安排（每日块→次日 E1 自动重建当日份；一次性/大任务→目标/承诺保持 pending 不归档，剩余由 09:30 容量提议/E1 继续排，确认邮件只信息性点明不施压）。23:00 记账：**当天任务完成（勾掉）≠ 目标完成（推进阶段/归档承诺/激活待激活）**——下一步动作整体完成才推进。
...
- **系统侧**：显式标记 → 直接采用，不推断；**未标 → P0（无优先级，不计入容量判断），确认邮件附「没标优先级默认 P0，要改回个数字即可」一句（2026-08-18 补）**；标记模糊（P2 等不存在值/冲突/看不懂）→ **不猜，在确认邮件里询问**。
- 滴答里手动改的 priority 永远优先于邮件初值（容量判断读滴答实时值）。

### 相对日期锚定规则（2026-08-18 部署：回复旧邮件时的日期解析）

事故：用户 8-18 早上回复昨晚 23:00 复盘邮件「明天需要了解调研…」（23:00 邮件问「明天最重要 1 件事」），09:30 槽按处理当天解析「明天」=8-19，实际语境=8-18。规则：

- 邮件是**对旧邮件的回复**时，回复中的相对日期（今天/明天/周末/下周）以**被回复邮件的语境**为基准，不以处理当天为基准。
- 推断出的日期在确认邮件/正文里**点明**（如「已按 8-18 记」），用户一眼可纠。
- 拿不准时在确认邮件里确认日期，不静默猜测。

### 承诺生命周期：待激活（2026-08-17 部署：前置未满足的承诺不进排程）

逻辑依赖问题：阶段前置任务（如调研）没完成时，后置任务（如刷题）不应被排——否则出现「还没教材就在刷题」。机制：

- `commitments.md` 的状态列支持 **`待激活`**：前置检查列写「#N 完成后激活」，该承诺**不进任何槽位候选**（E1/09:30/10:00 只抓 状态=pending，连续跳过统计也不计入）。
- **23:00 复盘自动激活**：阶段推进时检查 `待激活` 承诺，前置已满足（前置指向的承诺被归档/完成）→ 改回 `pending`。
- 首例：#6 每日行测刷题 → 待激活（前置 #4 网课调研完成）。
...
**未执行判定铁律（用户 2026-08-17 定，信号口径 2026-08-19 扩充）**：晚间块的「执行」只认用户确认信号——回复完成情况（「完成了最小版本」「做了X」「启动了/动了」）、完成报告、或明确让位声明（如「今晚看电影」）。**用户未回复 = 未执行**：daily 中带「默认」「未确认」「未回无信号」标记且无后续确认的块，一律按未执行处理，不计入覆盖次数。23:00 复盘汇总覆盖分布时同样遵守（全天无确认信号则如实呈现「0 块确认执行」）。
